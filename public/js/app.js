...
(function checkDataVersion(){
  ...
  if(stored<DATA_VERSION){/* ── FLUX PLANNER · app.js v2 ── */
...
function handleSignedOut(){
  ...
  // Clear guest flag so showLoginOrApp always shows login screen after sign out
  ...
  setSyncStatus('offline');
  // Always go to login screen on sign out — never drop into guest mode
  document.getElementById('app').classList.remove('visible');
  document.getElementById('loginScreen').classList.add('visible');
  initLoginFeaturePills();
}
