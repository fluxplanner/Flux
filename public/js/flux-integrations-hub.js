...
(function () {
  ...
  // B4: student-relevant tools lead (Google, Classroom, Canvas, Notion,
  // Todoist); dev/automation tools are marked `secondary` and collapse
  // behind "More tools" in the onboarding picker.
  const TILES = [
    { id: 'gsuite', label: 'G Suite', connectorId: 'google', svg: GOOGLE_G },
    { id: 'classroom', label: 'Classroom', connectorId: 'google', svg: '<svg viewBox="0 0 24 24"><rect width="24" height="24" rx="4" fill="#0F9D58"/><rect x="4" y="5" width="16" height="14" rx="1.2" fill="#F7F7F7"/><circle cx="12" cy="10.2" r="1.9" fill="#0F9D58"/><path d="M7.6 15.4c.7-1.7 2.4-2.6 4.4-2.6s3.7.9 4.4 2.6z" fill="#0F9D58"/><circle cx="7.2" cy="11" r="1.3" fill="#57BB8A"/><circle cx="16.8" cy="11" r="1.3" fill="#57BB8A"/></svg>' },
    { id: 'canvaslms', label: 'Canvas', connectorId: 'canvas', svg: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#E72429"/><circle cx="12" cy="12" r="3.2" fill="#fff"/><g fill="#fff"><circle cx="12" cy="4.6" r="1.3"/><circle cx="12" cy="19.4" r="1.3"/><circle cx="4.6" cy="12" r="1.3"/><circle cx="19.4" cy="12" r="1.3"/><circle cx="6.8" cy="6.8" r="1.1"/><circle cx="17.2" cy="6.8" r="1.1"/><circle cx="6.8" cy="17.2" r="1.1"/><circle cx="17.2" cy="17.2" r="1.1"/></g></svg>' },
    ...
    { id: 'jira', secondary: true, label: 'Jira', connectorId: 'jira', svg: '<svg viewBox="0 0 24 24"><path d="M12 2 6.5 7.5a2.6 2.6 0 0 0 0 3.7l5.5 5.5 5.5-5.5a2.6 2.6 0 0 0 0-3.7z" fill="#2684FF"/><path d="M12 8.5 8.9 11.6a2.6 2.6 0 0 0 0 3.7L12 18.4l3.1-3.1a2.6 2.6 0 0 0 0-3.7z" fill="#0052CC"/><path d="m12 15-1.7 1.7a2.6 2.6 0 0 0 0 3.6L12 22l1.7-1.7a2.6 2.6 0 0 0 0-3.6z" fill="#2684FF"/></svg>' },
    ...
    { id: 'github', secondary: true, label: 'GitHub', connectorId: 'github', svg: '<svg viewBox="0 0 24 24"><path fill="#181717" d="M12 .5C5.65.5.5 5.65.5 12c0 5.08 3.29 9.39 7.86 10.91.58.11.79-.25.79-.55 0-.27-.01-1.17-.02-2.12-3.2.7-3.88-1.36-3.88-1.36-.52-1.33-1.28-1.68-1.28-1.68-1.04-.71.08-.7.08-.7 1.15.08 1.76 1.18 1.76 1.18 1.03 1.76 2.69 1.25 3.35.96.1-.75.4-1.25.72-1.54-2.55-.29-5.24-1.28-5.24-5.68 0-1.26.45-2.28 1.18-3.09-.12-.29-.51-1.46.11-3.05 0 0 .96-.31 3.16 1.18a11 11 0 0 1 5.75 0c2.2-1.49 3.16-1.18 3.16-1.18.62 1.59.23 2.76.11 3.05.73.81 1.18 1.83 1.18 3.09 0 4.41-2.69 5.38-5.26 5.66.41.36.78 1.06.78 2.14 0 1.55-.01 2.79-.01 3.17 0 .31.21.67.8.55A11.51 11.51 0 0 0 23.5 12C23.5 5.65 18.35.5 12 .5z"/></svg>' },
    { id: 'linear', secondary: true, label: 'Linear', connectorId: 'linear', svg: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#5E6AD2"/><g stroke="#fff" stroke-width="1.7" stroke-linecap="round"><path d="M5 13.5 10.5 19"/><path d="M5 9.5 14.5 19"/><path d="M6.8 6.3 17.7 17.2"/><path d="M10 4.8 19.2 14"/></g></svg>' },
  ...
  function tileHTML(tile, mode) {
    ...
    // Picker shows the honest "Coming soon" state too — selecting a tile that
    // can't connect yet shouldn't look like a live integration (B4).
    const chip = mode === 'hub'
      ? statusChip(tile)
      : (tileStatus(tile) === 'coming_soon' ? '<span class="fih-chip">Coming soon</span>' : '');
    return `
  ...
  function renderPickerInto(el) {
    ...
    el.addEventListener('click', (e) => {
      const more = e.target.closest('.fih-more-toggle');
      if (more) {
        const sec = el.querySelector('.fih-grid-secondary');
        const open = sec && sec.hidden;
        if (sec) sec.hidden = !open;
        more.setAttribute('aria-expanded', open ? 'true' : 'false');
        more.textContent = open ? 'Fewer tools ▴' : 'More tools ▾';
        return;
      }
