...
test.describe('Palette & quick-add keyboard handling', () => {
  ...
  /* Originally typed "notes". That query no longer resolves — the Notebook tab
     is paused (FLUX_NOTEBOOK_ENABLED in app.js) — and the bug this guards was
     never about the destination: the leading letter fired a global shortcut and
     leaked into quick-add, creating a task named after whatever you typed. Any
     live tab exercises it, so this uses Calendar and keeps both assertions.
     "calendar" still leads with 'c', itself a global shortcut, so the guard
     keeps its teeth. */
  test('exact junk-task repro: ⌘K → "calendar" → Enter creates NO task', async ({ page }) => {
    ...
    const junk = await page.evaluate(() =>
      (window as any).tasks.some((t: any) => /^alendar$|^calendar$/i.test(t.name)));
